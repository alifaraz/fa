...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	if is_gbanned(msg.from.id) then
        return '<i>✨شما گولبال بن هستید✨.\n ✨برای ازاد کردن خود به @Me_sudo_MR_FOX  مراجعه کنید✨</i>'
	end
	...
	if matches[2]:lower() == 'english' and matches[3]:lower() == 'support' then
		...
		if is_banned(msg.from.id, tostring(target)) then
			return '<i>✨شما بن هستید.برای ازاد کردن خود به @Me_sudo_MR_FOX  مراجعه کنید✨</i>'
		end
	...
	if matches[1] == 'help' and msg.to.type == 'user' or matches[1] == 'pmhelp' and is_admin1(msg) and msg.to.type ~= 'user' then
		...
		text = " <i>✨خوش آمدید✨!\n\n>برای دریافت گروه یا مدیریت گروه شما  یا ربات @Me_sudo_MR_FOX مراجعه کنید! یا بایکی از مدیران در تماس باشید, برای اینکار به آیدی ادمین مراجعه کنبد:\n[ @Me_sudo_MR_FOX ] \n\nبااستفاده از دستور 'راهنما' لیست دستورات را دریافت کنید!!\n\n✨باتشکر از انتخاب @Me_sudo_MR_FOX ✨ </i>"
     	return text
    end
